...
class VectorizedSampler(ModelBaseSampler):
    ...
    def start_worker(self):
        n_envs = self.n_envs
        if n_envs is None:
            n_envs = int(self.algo.batch_size / self.algo.model_max_path_length)
            n_envs = max(1, min(n_envs, 100))

        if getattr(self.algo.env, 'vectorized', False):
            self.vec_env = self.algo.env.vec_env_executor(n_envs=n_envs, max_path_length=self.algo.model_max_path_length)
        else:
    ...
    def obtain_samples(self, itr, observations, log=True, log_prefix=''):
        logger.log("Obtaining samples for iteration %d..." % itr)
        paths = []
        n_samples = 0
        self.initial_observations = np.repeat(observations, self.num_branches, axis=0)
        batch_size = len(observations) * self.num_branches * self.algo.model_max_path_length
        pbar = ProgBarCounter(batch_size)
        self.vec_env.reset_args = self.initial_observations
