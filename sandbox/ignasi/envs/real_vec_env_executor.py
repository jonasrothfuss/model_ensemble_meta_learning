...
class VecEnvExecutor(object):
    ...
    def step(self, action_n):
        ...
        for (i, done) in enumerate(dones):
            if done:
                obs[i] = self.envs[i]._wrapped_env._wrapped_env.reset(self.reset_args[self._i])
                self._i += 1
                self.ts[i] = 0
        return obs, rewards, dones, tensor_utils.stack_tensor_dict_list(env_infos)

    def reset(self):
        results = [env._wrapped_env._wrapped_env.reset(self.reset_args[i]) for i, env in enumerate(self.envs)]
        self._i = self.num_envs
        self.ts[:] = 0
        return results
