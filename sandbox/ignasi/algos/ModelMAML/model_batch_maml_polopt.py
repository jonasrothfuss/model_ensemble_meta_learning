...
class ModelBatchMAMLPolopt(RLAlgorithm):
    ...
    def __init__(
        ...
        :param use_vine: Whether to use VINE to collect the policy
        :return:
        """
        self.env = env
        self.policy = policy
        ...
        if self.use_vine:
            self.model_sampler = MAMLVINEModelVectorizedSampler(self)
            self.policy_sampler_processor = VINEModelBaseSampler(self)
        else:
    ...
    def get_init_obs(self, data, mode='last'):
        size = self.n_vine_init_obs
        if mode == 'last':
            observations = data['observations_dynamics']
                ...
                with logger.prefix('itr #%d | ' % itr):
                    ...
                    if self.log_real_data:
                        ...
                        for key in new_env_paths.keys():
                            samples_data[key] = self.process_samples_for_policy(itr, new_env_paths[key],
                                                                                use_vine=False, log=False)
