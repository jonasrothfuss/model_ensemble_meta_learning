...
class SawyerPickPlaceEnv(MultitaskEnv, SawyerXYZEnv):
    def __init__(
            ...
            fix_goal=True,
            fixed_goal=(0, 0.85, 0.02), #3D placing goal, followed by height target for picking
            height_target=0.1,
            goal_low=None,
            goal_high=None,
    ...
    def step(self, action):
        ...
        return obs, reward, done, {'pickRew':pickRew, 'placeRew': placeRew}
...
if __name__ == "__main__":
    env = SawyerPickPlaceEnv(fix_goal=True)
    import time
    env.reset()
    for _ in range(1000):
        env.render()
