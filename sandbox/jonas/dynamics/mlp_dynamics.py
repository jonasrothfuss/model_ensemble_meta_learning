...
class MLPDynamicsModel(LayersPowered, Serializable):
    ...
    def fit(self, obs, act, obs_next, epochs=100, verbose=False):
        ...
        for epoch in range(epochs):
            ...
            sess.run(iterator.initializer,
                          feed_dict={self.obs_dataset_ph: obs, self.act_dataset_ph: act, self.next_obs_dataset_ph: obs_next})
    ...
    def _data_input_fn(self, obs, act, obs_next, batch_size=500, buffer_size=100000):
        ...
        dataset = tf.data.Dataset.from_tensor_slices((self.obs_dataset_ph, self.act_dataset_ph, self.next_obs_dataset_ph))
