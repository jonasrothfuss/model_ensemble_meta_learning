...
from sandbox.dennis.algos.MAML.maml_ppo import MAMLPPO as DennisMAMLPPO
from sandbox.ignasi.algos.MAML.maml_ppo import MAMLPPO as IgnasiMAMLPPO
...
def run_experiment(argv):
    ...
    for v, exp_id in zip(variants, exp_ids):
        exp_name = "ppo_maml_train_rand_goal_env_%s_%.1f_%.3f_%i_%i_id_%i" %(v['env'],
                                                                             v['clip_eps'],
                                                                             v['target_inner_step'],
                                                                             v['max_epochs'],
                                                                             v['seed'], exp_id)
...
def instantiate_class_stings(v):
    v['env'] = globals()[v['env']]
    if v['code_base'] == 'ignasi':
        v['policy'] = globals()['IgnasiPolicy']
        v['algo'] = globals()['IgnasiMAMLPPO']
    elif v['code_base'] == 'dennis':
        v['policy'] = globals()['DennisPolicy']
        v['algo'] = globals()['DennisMAMLPPO']
