...
def run_train_task(vv):
    ...
    dynamics_model = PointEnvFakeModelEnsemble(
        ...
        error_range_around_goal=vv['fake_model_setup']['error_range_around_goal'],
...
def run_experiment(argv):
    ...
    vg.add('point_env_setup', [
        {'init_sampling_boundaries': (2, 6), 'goal': (4, 4)},
    ])

    # fake model spec
    vg.add('fake_model_setup', [
        {'error_range_around_goal': 0.5, 'bias_range': 0.05, 'error_std': 0.01, 'error_at_goal': False},
        {'error_range_around_goal': 0.5, 'bias_range': 0.01, 'error_std': 0.001, 'error_at_goal': False},
    ])
    vg.add('smooth_error', [True])
