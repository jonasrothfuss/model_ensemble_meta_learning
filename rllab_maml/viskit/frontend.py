...
from rllab_maml.misc.ext import flatten
from rllab_maml.viskit import core
...
def make_plot_eps(plot_list, use_median=False, counter=0):
    ...
    for idx, plt in enumerate(plot_list):
        ...
        if use_median:
            ...
            y_upper = list(plt.means + plt.stds)
            y_lower = list(plt.means - plt.stds)
        plt.legend = plt.legend.replace('rllab_maml.algos.trpo.TRPO', 'TRPO')
        plt.legend = plt.legend.replace('rllab_maml.algos.vpg.VPG', 'REINFORCE')
        plt.legend = plt.legend.replace('rllab_maml.algos.erwr.ERWR', 'ERWR')
        plt.legend = plt.legend.replace('sandbox_maml.rein.algos.trpo_vime.TRPO', 'TRPO+VIME')
        plt.legend = plt.legend.replace('sandbox_maml.rein.algos.vpg_vime.VPG', 'REINFORCE+VIME')
        plt.legend = plt.legend.replace('sandbox_maml.rein.algos.erwr_vime.ERWR', 'ERWR+VIME')
